...
https://jrnold.github.io/r4ds-exercise-solutions/data-import.html#exercises-14 :
...
"Only two columns are specified in the header “a” and “b”, but the rows have three columns, so the last column in dropped" -- https://jrnold.github.io/r4ds-exercise-solutions/data-import.html#exercises-14

A5.2
Row1 has three positions, row2 two positions, row3 four positions. Created 2x3 tibble (NA, no 4).
"The numbers of columns in the data do not match the number of columns in the header (three). In row one, there are only two values, so column c is set to missing. In row two, there is an extra value, and that value is dropped" -- https://jrnold.github.io/r4ds-exercise-solutions/data-import.html#exercises-14

A5.3
Row1 has two positions, row2 has \"1. Created 1x2 tibble (with 1 in first position, NA in second)
...
A5.5
Uses ; [`read_csv2` uses ; as a delimiter instead of ,]. Created 1x1 tibble
...
* encoding: [`encoding`]
-- https://jrnold.github.io/r4ds-exercise-solutions/data-import.html#exercises-15
...
"Locales also provide default date and time formats. The time format isn’t currently used for anything, but the date format is used when guessing column types" -- https://cran.r-project.org/web/packages/readr/vignettes/locales.html

...
I think "hard to understand" refers to the x-axis labels being squashed/overlapping/unreadable. Usual way to overcome this (unless bars have to be vertical) is to make the bars horizontal :
```{r}
forcats::gss_cat %>%
  ggplot(aes(rincome)) +
  geom_bar() +
...
Other improvements : descending values (re. reading down list), consistent bin size (but we can't affect that) ...and maybe grouping some 'no answer' categories, if no values can be got from them
...
levels(forcats::gss_cat$denom)     # 30 different values. Answer is Protestant

...
I think "hard to understand" is referring to the x-axis labels being squashed/overlapping/unreadable. Usual way to overcome this (unless bars have to be vertical) is to make the bars horizontal :
```{r}
forcats::gss_cat %>%
  ggplot(aes(rincome)) +
  geom_bar() +
...
Other improvements : descending values (re. reading down list), consistent bin size (but we can't affect that) ...and maybe to group together some of the 'no answer' categories, if no values can be got from them
...
A3.
```{r}
forcats::gss_cat$denom             # Values for the first 1000 rows. Answer appears to be Protestant
levels(forcats::gss_cat$denom)     # There are 30 different values. Answer is Protestant
...
# Answer from [https://jrnold.github.io/r4ds-exercise-solutions/factors.html#general-social-survey] includes quantities as size, but its most useful feature is the rotation of labels on the x-axis :
gss_cat %>%
  count(relig, denom) %>%
...
[https://jrnold.github.io/r4ds-exercise-solutions/factors.html#modifying-factor-order]
...
levels(forcats::gss_cat$rincome)   # Shows there are 16 different values in total

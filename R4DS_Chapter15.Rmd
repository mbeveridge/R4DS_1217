...
Other improvements possible : descending values (re. reading down list), consistent bin size (but we can't affect that) ...and maybe grouping some 'no answer' categories, if no values can be got from them
...
forcats::gss_cat %>%
  ...
  geom_point() +
  coord_flip()  # Incl/excl this, to read both axis labels. (Q tells only 1 answer)
...
gss_cat %>%
  filter(!is.na(tvhours)) %>%      # extra line means no 'Warning' (but same chart)
  ggplot(aes(x = tvhours)) +
  geom_histogram(binwidth = 1)
```

...
A1.
From [https://jrnold.github.io/r4ds-exercise-solutions/model-basics.html#visualizing-models]

* "I’ll use `add_predictions` to add the predictions from a `loess` regression to the `sim1 data`"
```{r}
sim1_loess <- loess(y ~ x, data = sim1)

grid_loess <- sim1 %>%
  add_predictions(sim1_loess)                          # `add_predictions`

# add the predictions from a `loess` regression to the `sim1` data
# sim1 <- sim1 %>%
#  add_predictions(sim1_loess, var = "pred_loess")     # Default name of output col w/be `pred`
```

"This plots the `loess` predictions. The `loess` produces a nonlinear, smooth line through the data"
```{r}
ggplot(sim1, aes(x)) +                 # Same code as s23.3.1 R4DS book (but with `grid_loess`)
  geom_point(aes(y = y)) +

...
* `extract()` uses a regular expression to find groups and split one column into many (multiple ways possible).
* `separate()` : uses `sep` to split one column into many (multiple ways possible). Default value is a regular expression that matches any sequence of non-alphanumeric values. Else, you can pass a specific character (to be interpreted as a regular expression), or a vector of integers (interpreted as positions to split at)
...
NOTE : Not sure when you'd choose `extract()` instead of `separate()`

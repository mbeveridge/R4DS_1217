...
A4. The plot does use all the aesthetics. But there is redundancy, which may be confusing (as a reader expects that an aesthetic is added for extra insight)
...
```
Expression is True/False for each point, splitting `displ` into 2 sets with different colours
...
?geom_smooth
```
"display confidence interval around smooth? (TRUE by default, see level to control" ...ie. adds error bands to a line
...
ggplot(data = mpg, mapping = aes(x = displ, y = hwy)) +
  ...
  geom_point(mapping = aes(color = drv))
```
* Note the white points are in the earlier layer, with the colours then on top
* Instead of `stroke=2` could have used `size=4` (from seeing someone else's answer)
* Actually, why did 'stroke' work? - does it not need to be with a shapetype that can have color+fill?
...
Default geom for `stat_summary` is `geom_pointrange`. But default stat for `geom_pointrange` is `identity` (which is NOT what we want), so specify `geom_pointrange(stat = "summary")` [see below] :
```{r}
ggplot(data = diamonds) +
...
`geom_count()` makes larger points where there is overplotting, so the number of visible points is equal to `geom_point()`
...
ggplot(data = mpg, mapping = aes(x = cty, y = hwy)) +
  ...
  coord_fixed() # draws equal intervals on \(x\) & \(y\) axes, so more easily comparable (`abline` at 45 degrees)
```

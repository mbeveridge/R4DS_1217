...
A3.
This refers to `bad_fit` data frame, which is plotted as a line chart, at the end of s25.2.4 R4DS book

An earlier step (in s25.2.4 R4DS book) towards the `bad_fit` data frame tried `unnest(glance)`, but "This isn’t quite the output we want, because it still includes all the list columns. This is default behaviour when `unnest()` works on single row data frames. To suppress these columns we use `.drop = TRUE`" ...And then the book used `unnest(glance, .drop = TRUE)` instead

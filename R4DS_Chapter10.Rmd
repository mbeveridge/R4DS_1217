...
reports its type, a nice feature borrowed from `str()`" -- s10.3.1 book text
...
generate a warning if the column you are trying to access does not exist" -- s10.3.2 book text
...
With tibbles, [ always returns another tibble" -- s10.4 book text
...
a space. To refer to these variables, you need to surround them with backticks, ` -- s10.2 book text
...
A6.
"you can explicitly `print()` the data frame and control the number of rows (`n`) and the width of the display. `width = Inf` will display all columns" -- s10.3.1 book text
...
Use `options(tibble.width = Inf)` to always print all columns, regardless of the width of the screen" -- s10.3.1 book text
